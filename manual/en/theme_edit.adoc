...
This guide helps to write custom HoustonPatchbay patchbay themes, which are used by NexSession, Patchance and Patchichi.
...
* _software_ with the software you use in lower case (nexsession, patchance or patchichi)
...
* *client* : NexSession client boxes, if JACK client name follows the NSM rules.
...
That said, in Patchance or NexSession, portgroups exists only for regular audio ports (for the moment, but are midi or CV portgroups really needed ??).
